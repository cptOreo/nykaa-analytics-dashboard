        ...
        html.Div(className='main-column', children=[
            ...
            html.Div(className='hairline-top flex-row', children=[
                html.Div(className='flex-1', children=[
                    c.chart_card(dcc.Graph(figure=c.trend_chart(YEARS, [_safe(_kpis(y, 'Fashion').get('Contribution Margin %')) for y in YEARS], [_safe(_kpis(y, 'Beauty').get('Contribution Margin %')) for y in YEARS], "CONTRIBUTION MARGIN", "%", annotate_gap=True, yaxis_range=[-5, 30])))
                ]),
                html.Div(className='flex-1', children=[
                    c.chart_card(dcc.Graph(figure=c.trend_chart(YEARS, [_safe(_kpis(y, 'Fashion').get('Marketing + S&D %')) for y in YEARS], [_safe(_kpis(y, 'Beauty').get('Marketing + S&D %')) for y in YEARS], "COMMERCIAL SPEND", "%", annotate_gap=True, yaxis_range=[-5, 30])))
                ])
            ])
        ])
    ])
    ...
    return html.Div(className='editorial-grid', children=[
        ...
        html.Div(className='main-column', children=[
            ...
            html.Div(style={'marginTop': '40px'}, children=[
                c.chart_card(dcc.Graph(figure=c.create_gap_plot(['Return Rate', 'Repeat Purchase', 'Repurchase Intent (1-5)'], [f_ret, f_rep, df[df['category']=='Fashion']['repurchase_intent_3m_1_5'].mean()], [b_ret, b_rep, df[df['category']=='Beauty']['repurchase_intent_3m_1_5'].mean()], "BEHAVIOURAL GAPS", formatter=c.fmt_number)))
            ])
        ])
    ])
...
def build_page_scenarios(year):
    return html.Div(className='editorial-grid', children=[
        ...
        html.Div(className='main-column', children=[
            ...
            html.Div(id='scenario-results', style={'marginTop': '24px'})
        ])
    ])
